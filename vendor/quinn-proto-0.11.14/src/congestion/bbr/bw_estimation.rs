...
mod tests {
    ...
    #[test]
    fn app_limited_sample_can_raise_bandwidth_estimate() {
        let mut estimate = BandwidthEstimation::default();
        let start = Instant::now();

        estimate.on_sent(start, 1_000);
        estimate.on_ack(start, start, 1_000, 0, false);
        estimate.on_sent(start + Duration::from_millis(10), 1_000);
        estimate.on_ack(
        ...
        let initial = estimate.get_estimate();
        assert!(initial > 0);

        estimate.on_sent(start + Duration::from_millis(21), 1_000);
        estimate.on_ack(
        ...
        assert!(estimate.get_estimate() > initial);
    }

    #[test]
    fn non_app_limited_samples_age_bandwidth_estimate() {
        let mut estimate = BandwidthEstimation::default();
        let start = Instant::now();
        let initial = 1_000_000;
        estimate.max_filter.update_max(0, initial);
        estimate.on_sent(start, 1_000);
        estimate.on_ack(start, start, 1_000, 0, false);

        for round in 1..=11 {
            let sent = start + Duration::from_millis(round * 20);
            estimate.on_sent(sent, 1_000);
            estimate.on_ack(sent + Duration::from_millis(10), sent, 1_000, round, false);
        }

        assert!(estimate.get_estimate() < initial);
        assert!(estimate.get_estimate() > 0);
    }
}
